    ...
    public override func layoutWith(layout: QuickListCollectionLayout, section: Section, currentStart: CGPoint) -> QuickListSectionAttribute {
        ...
        if let header = section.header {
            ...
            if layout.scrollDirection == .vertical {
                ...
                let headerHeight = header.height(section, CGSize(width: maxHeight, height: maxHeight), layout.scrollDirection)
                frame = CGRect(x: tempStart.x, y: formContentInset.top, width: headerHeight, height: maxHeight)
                tempStart.x += headerHeight
        ...
        if let footer = section.footer {
            ...
            if layout.scrollDirection == .vertical {
                ...
                let footerHeight = footer.height(section, CGSize(width: maxHeight, height: maxHeight), layout.scrollDirection)
                frame = CGRect(x: tempStart.x, y: formContentInset.top, width: footerHeight, height: maxHeight)
                tempStart.x += footerHeight
            }
        ...
        if section.decoration != nil {
            let decorationAttributes = UICollectionViewLayoutAttributes(forSupplementaryViewOfKind: SectionReusableType.decoration.elementKind, with: IndexPath(item: 0, section: sectionIndex))
            var frame: CGRect = .zero
            if layout.scrollDirection == .vertical {
